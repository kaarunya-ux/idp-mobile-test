...
struct clip_ctx {
    ...
    int warmup_alloc_w = 0;
    int warmup_alloc_h = 0;
...
// returns tensor with shape [n_embd, n_patches, n_batch]
...
struct clip_model_loader {
    ...
    void load_tensors(clip_ctx & ctx_clip) {
        ...
        for (int il = 0; il < n_layers_to_load; ++il) {
            ...
            bool is_ffn_swapped = (
                    ...
                    || model.proj_type == PROJECTOR_TYPE_CUSTOM
    ...
    static void warmup(clip_ctx & ctx_clip, const clip_image_f32_batch & batch) {
        ...
        if (!batch.entries.empty()) {
            ctx_clip.warmup_alloc_w = batch.entries[0].nx();
            ctx_clip.warmup_alloc_h = batch.entries[0].ny();
        }
...
bool clip_image_batch_encode(clip_ctx * ctx, int n_threads, const clip_image_f32_batch * imgs_c_ptr, std::vector<float> & out_batch_embd) {
    ...
    const int w = imgs.entries[0].nx();
    const int h = imgs.entries[0].ny();
    if (ctx->is_allocated && (w != ctx->warmup_alloc_w || h != ctx->warmup_alloc_h)) {
