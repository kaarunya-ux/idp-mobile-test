...
int32_t mtmd_helper_eval_chunks(mtmd_context * ctx,
    ...
    auto run_legacy = [&]() -> int32_t {
        for (size_t i = 0; i < n_chunks; i++) {
            bool chunk_logits_last = (i == n_chunks - 1) && logits_last;
            auto chunk = mtmd_input_chunks_get(chunks, i);
            int32_t res = mtmd_helper_eval_chunk_single(
                ctx, lctx, chunk, n_past, seq_id, n_batch,
    ...
    const bool batch_enabled = (std::getenv("MTMD_DISABLE_BATCH_SLICES") == nullptr);
    if (!batch_enabled) {
        return run_legacy();
    }

    std::vector<const mtmd_input_chunk *> image_chunks;
    image_chunks.reserve(n_chunks);
    int32_t n_image_tokens_total = 0;
    for (size_t i = 0; i < n_chunks; i++) {
        auto * c = mtmd_input_chunks_get(chunks, i);
        if (mtmd_input_chunk_get_type(c) == MTMD_INPUT_CHUNK_TYPE_IMAGE) {
            image_chunks.push_back(c);
            n_image_tokens_total += (int32_t) mtmd_input_chunk_get_n_tokens(c);
        }
    }

    const bool no_upscale_env = []() {
        const char * env = std::getenv("MTMD_NO_UPSCALE");
        return env && (env[0] == '1' || env[0] == 't' || env[0] == 'T' || env[0] == 'y' || env[0] == 'Y');
    }();
    if (image_chunks.size() <= 1 && !no_upscale_env) {
        return run_legacy();
    }
    ...
    LOG_INF("encoding image slices in batched mode (n_image_chunks=%zu, n_tokens=%d)...\n",
            image_chunks.size(), n_image_tokens_total);

    int64_t t0 = ggml_time_ms();

    std::vector<std::pair<const mtmd_input_chunk *, float *>> embd_for;
    embd_for.reserve(image_chunks.size());

    std::vector<mtmd_batch *> batches;
    batches.reserve(4);

    auto free_all_batches = [&]() {
        for (auto * b : batches) {
            mtmd_batch_free(b);
        }
        batches.clear();
    ...
    mtmd_batch * cur = mtmd_batch_init(ctx);
    if (!cur) {
        LOG_ERR("mtmd_batch_init failed; falling back to legacy\n");
        return run_legacy();
    }
    batches.push_back(cur);

    auto cache_embd_for_batch = [&](mtmd_batch * b) -> int32_t {
        size_t n = mtmd_batch_n_chunks(b);
        for (size_t j = 0; j < n; j++) {
            const mtmd_input_chunk * c = mtmd_batch_get_chunk(b, j);
            float * e = mtmd_batch_get_output_embd(b, c);
            if (!e) {
                LOG_ERR("mtmd_batch_get_output_embd returned null for chunk %zu in batch %zu\n",
                        j, batches.size() - 1);
                return 1;
            }
            embd_for.emplace_back(c, e);
        }
        return 0;
    };

    size_t idx = 0;
    while (idx < image_chunks.size()) {
        int32_t rc = mtmd_batch_add_chunk(cur, image_chunks[idx]);
        if (rc == 0) {
            idx++;
            continue;
        }
        if (rc != 2 && rc != 3) {
            LOG_ERR("mtmd_batch_add_chunk failed rc=%d at idx=%zu\n", rc, idx);
            free_all_batches();
            return run_legacy();
        }
        int32_t enc = mtmd_batch_encode(cur);
        if (enc != 0) {
            LOG_ERR("mtmd_batch_encode failed rc=%d\n", enc);
            free_all_batches();
            return run_legacy();
        }
        if (cache_embd_for_batch(cur) != 0) {
            free_all_batches();
            return run_legacy();
        }
        cur = mtmd_batch_init(ctx);
        if (!cur) {
            LOG_ERR("mtmd_batch_init failed on flush\n");
            free_all_batches();
            return run_legacy();
        }
        batches.push_back(cur);
        int32_t rc2 = mtmd_batch_add_chunk(cur, image_chunks[idx]);
        if (rc2 != 0) {
            LOG_ERR("add on fresh batch returned %d\n", rc2);
            free_all_batches();
            return run_legacy();
        }
        idx++;
    ...
    int32_t enc = mtmd_batch_encode(cur);
    if (enc != 0) {
        LOG_ERR("mtmd_batch_encode (final) failed rc=%d\n", enc);
        free_all_batches();
        return run_legacy();
    }
    if (cache_embd_for_batch(cur) != 0) {
    ...
    int64_t encode_ms = ggml_time_ms() - t0;
    LOG_INF("image slices encoded in %" PRId64 " ms (batched, n_batches=%zu)\n",
            encode_ms, batches.size());

    auto lookup_embd = [&](const mtmd_input_chunk * c) -> float * {
        for (auto & kv : embd_for) {
    ...
    for (size_t i = 0; i < n_chunks; i++) {
        auto * c = mtmd_input_chunks_get(chunks, i);
        auto ct = mtmd_input_chunk_get_type(c);
        bool chunk_logits_last = (i == n_chunks - 1) && logits_last;

        if (ct == MTMD_INPUT_CHUNK_TYPE_IMAGE) {
            float * embd = lookup_embd(c);
            if (!embd) {
                LOG_ERR("missing pre-computed embd for image chunk %zu — "
                        "this should never happen\n", i);
                free_all_batches();
                return 1;
            }
            llama_pos local_new;
            int32_t dec = mtmd_helper_decode_image_chunk(
                ctx, lctx, c, embd, n_past, seq_id, n_batch,
                &local_new, nullptr, nullptr);
            if (dec != 0) {
                LOG_ERR("decode failed for IMAGE chunk %zu in main pass\n", i);
                free_all_batches();
                return dec;
            }
            n_past = local_new;
            *new_n_past = n_past;
            ...
            // TEXT or AUDIO: legacy path (text decode, or audio encode+decode).
            int32_t res = mtmd_helper_eval_chunk_single(
                ctx, lctx, c, n_past, seq_id, n_batch,
                chunk_logits_last, &n_past);
            if (res != 0) {
                LOG_ERR("failed to eval chunk %zu (legacy single)\n", i);
                free_all_batches();
                return res;
            }
            *new_n_past = n_past;
