...
mtmd_image_preproc_out mtmd_image_preprocessor_dyn_size::preprocess(const clip_image_u8 & img) {
    ...
    // Experimental: let a tight single-field crop and a full document photo use
    // independent pixel budgets instead of one fixed GGUF-baked min/max, auto-selected
    // by the native image's own pixel area - no extra request field needed, since a
    // crop and a full photo are trivially distinguishable by size alone. Off by
    // default (falls through to stock hparams values when unset), so every other
    // model using this preprocessor (GLM4V, Gemma4V, etc.) is unaffected unless these
    // are explicitly set. Threshold defaults to this model's own image_min_pixels -
    // its own "comfort floor" - so anything at or below it is unambiguously crop-sized.
    long min_pixels = hparams.image_min_pixels;
    long max_pixels = hparams.image_max_pixels;
    const long native_area = (long) original_size.width * (long) original_size.height;
    long crop_threshold = hparams.image_min_pixels;
    if (const char * env = std::getenv("MTMD_CROP_THRESHOLD_PIXELS")) {
        long v = std::atol(env);
        if (v > 0) crop_threshold = v;
    }
    const bool is_full_image = native_area > crop_threshold;
    const char * min_env = std::getenv(is_full_image ? "MTMD_FULLIMAGE_MIN_PIXELS" : "MTMD_CROP_MIN_PIXELS");
    const char * max_env = std::getenv(is_full_image ? "MTMD_FULLIMAGE_MAX_PIXELS" : "MTMD_CROP_MAX_PIXELS");
    if (min_env) { long v = std::atol(min_env); if (v > 0) min_pixels = v; }
    if (max_env) { long v = std::atol(max_env); if (v > 0) max_pixels = v; }
...
mtmd_image_preproc_out mtmd_image_preprocessor_idefics3::preprocess(const clip_image_u8 & img) {
    ...
    const int cur_merge = hparams.n_merge == 0 ? 1 : hparams.n_merge;
    const int patch_align = std::max(1, hparams.patch_size * cur_merge);

    clip_image_size refined_size;
    if (no_upscale) {
        const int max_orig_dim = std::max(original_size.width, original_size.height);
        effective_longest_edge = std::min(effective_longest_edge, max_orig_dim);
        refined_size = img_tool::calc_size_preserved_ratio(
            original_size, patch_align, effective_longest_edge);
    } else {
        refined_size = img_tool::calc_size_preserved_ratio(
            original_size, hparams.image_size, effective_longest_edge);
    }
    ...
    if (no_upscale) {
        if (instructions.grid_size.width == 1 && instructions.grid_size.height == 1) {
            instructions.overview_size = refined_size;
        } else {
            const int thumb_edge = std::min(
                static_cast<int>(hparams.image_size),
                std::max(refined_size.width, refined_size.height));
            instructions.overview_size = img_tool::calc_size_preserved_ratio(
                refined_size, patch_align, thumb_edge);
        }
    } else {
