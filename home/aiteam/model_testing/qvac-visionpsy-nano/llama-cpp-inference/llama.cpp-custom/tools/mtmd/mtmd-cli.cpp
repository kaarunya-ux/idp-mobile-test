...
struct mtmd_cli_context {
    ...
    void init_vision_context(common_params & params) {
        ...
        const char * env_bmt = std::getenv("MTMD_BATCH_MAX_TOKENS");
        mparams.batch_max_tokens = env_bmt ? std::atoi(env_bmt) : 4096;

        if (std::getenv("MTMD_DISABLE_BATCH_SLICES") == nullptr
            && std::getenv("MTMD_FORCE_BATCH") == nullptr) {
            for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
                ggml_backend_dev_t dev  = ggml_backend_dev_get(i);
                if (!dev) continue;
                const char * desc = ggml_backend_dev_description(dev);
                const char * name = ggml_backend_dev_name(dev);
                auto contains = [](const char * s, const char * sub) {
                    return s && sub && std::strstr(s, sub) != nullptr;
                };
                auto has = [&](const char * sub) {
                    return contains(desc, sub) || contains(name, sub);
                };
                const bool is_adreno_blacklist =
                       (has("Adreno") && has("830"))
                    || (has("Adreno") && has("740"));
                if (is_adreno_blacklist) {
                    LOG_WRN("Adreno GPU detected (name='%s', desc='%s') — "
                            "forcing MTMD_DISABLE_BATCH_SLICES=1\n",
                            name ? name : "?", desc ? desc : "?");
                    setenv("MTMD_DISABLE_BATCH_SLICES", "1", /*overwrite=*/0);
                    break;
                }
            }
        }
