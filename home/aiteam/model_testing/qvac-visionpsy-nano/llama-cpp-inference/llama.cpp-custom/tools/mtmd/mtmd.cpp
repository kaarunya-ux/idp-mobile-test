...
struct mtmd_context {
    ...
    bool ov_only_for_unsliced = false;
    ...
    void init_vision() {
        ...
        switch (proj) {
            ...
            case PROJECTOR_TYPE_CUSTOM:
                {
                    slice_tmpl             = MTMD_SLICE_TMPL_IDEFICS3;
                    tok_ov_img_start       = {lookup_token("<|global_image|>")};
                    tok_ov_img_end         = {};
                    tok_row_end            = {};
                    sli_img_start_tmpl     = "<row_%d_col_%d>";
                    ov_img_first           = true;
                    ov_only_for_unsliced   = true;
                    image_preproc = std::make_unique<mtmd_image_preprocessor_idefics3>(ctx_v);
                } break;
    ...
    int32_t add_media(std::vector<const mtmd_bitmap *> & bitmaps) {
        ...
        if (is_vision) {
            ...
            if (has_tiling_grid) {
                ...
                // add slices (or tiles)
                const bool skip_single_tile = ctx->ov_only_for_unsliced && n_row == 1 && n_col == 1;
                if (!chunks.empty() && !skip_single_tile) {
...
size_t mtmd_batch_n_chunks(const mtmd_batch * batch) {
    return batch ? batch->entries.size() : 0;
}

const mtmd_input_chunk * mtmd_batch_get_chunk(const mtmd_batch * batch, size_t idx) {
    if (!batch || idx >= batch->entries.size()) {
        return nullptr;
    }
