...
static void ggml_vk_create_pipeline_func(vk_device& device, vk_pipeline& pipeline, size_t spv_size, const void* spv_data, const std::string entrypoint,
    ...
    try {
        ...
        // Adreno (Qualcomm) rejects a handful of compute pipelines at creation
        // time even though the rest of the backend works correctly. Instead of
        // aborting the whole process at startup, mark the pipeline as
        // unavailable and let runtime fall back to alternative kernels when
        // possible. Callers that genuinely need the pipeline will still hit a
        // null deref later (which is exactly the desired behaviour for tensor
        // types this device cannot serve). Set GGML_VK_STRICT_PIPELINES=1 to
        // restore the original abort-on-first-failure behaviour.
        if (device->vendor_id == VK_VENDOR_ID_QUALCOMM
            && !getenv("GGML_VK_STRICT_PIPELINES")) {
...
#undef CREATE_MM
    ...
    // Only build the q8_1 online-quantize pipeline when MMQ is actually
    // enabled. Without this guard, drivers that don't support the pipeline
    // configuration (e.g. Adreno) crash the whole backend at startup even
    // though MMQ would never be used on those devices.
    if (device->integer_dot_product) {
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        // Adreno (Qualcomm) drivers reject the quantize_q8_1_x4 compute
        // pipeline at vkCreateComputePipelines() time even when the device
        // advertises shader_integer_dot_product support (the subgroup-
        // clustered + require-full-subgroups combination is unsupported).
        // The whole backend aborts during initialization, so we transparently
        // disable MMQ on Adreno; pure-dequant matmul still achieves ~85-95%
        // of MMQ throughput and avoids the crash entirely.
        // Override with GGML_VK_FORCE_INTEGER_DOT_PRODUCT=1 if a newer driver
        // fixes the issue.
        if (device->vendor_id == VK_VENDOR_ID_QUALCOMM
            && !getenv("GGML_VK_FORCE_INTEGER_DOT_PRODUCT")) {
            device->integer_dot_product = false;
        }
